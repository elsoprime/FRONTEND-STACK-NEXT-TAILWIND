# Operaciones Frontend

## Proposito

Centralizar documentos operativos de ejecucion, cierres por ola y runbooks del frontend para retomar trabajo sin ruido.

## Referencia activa de Expenses

- `EXPENSES_HOTFIX_CREATE_REQUEST_CIERRE_2026-03-22.md`
  Cierre vigente del hotfix de runtime para create-request y alineacion de `expenseDate` a ISO.

- `EXPENSES_OLA5_METRICAS_NATIVAS_CIERRE_2026-03-22.md`
  Cierre vigente de Ola 5 para metricas nativas, dashboard backend y migracion del tab reports.

- `EXPENSES_OLA6_SUBCATEGORIAS_REALES_CIERRE_2026-03-22.md`
  Cierre vigente de subcategorias reales en frontend (settings + request form + e2e focal).

## Checklist Maestro de Expenses

- `EXPENSES_CHECKLIST_MAESTRO_INTEGRACION_2026-03-22.md`
  Plan maestro y checklist de control para futuras olas, dependencias cruzadas y protocolo de retoma sin mezclarlo con cierres operativos.

## Propuesta de Refactor de Expenses

- `EXPENSES_PROPUESTA_REFACTORIZACION_TAXONOMIA_Y_RUNTIME_2026-03-22.md`
  Propuesta ejecutable para corregir la brecha de create-request y evolucionar a categorias/subcategorias reales sin deuda tecnica.

## Secuencia recomendada para retomar Expenses

1. `EXPENSES_PROPUESTA_IMPLEMENTACION_SAAS_2026-03-21.md`
2. `EXPENSES_OLA1_CATALOGO_CLP_CIERRE_2026-03-22.md`
3. `EXPENSES_OLA2_CATEGORIAS_MASIVAS_CIERRE_2026-03-22.md`
4. `EXPENSES_OLA3_DASHBOARD_CIERRE_2026-03-22.md`
5. `EXPENSES_OLA4_CATALOGO_GOBERNANZA_CIERRE_2026-03-22.md`
6. `EXPENSES_OLA5_METRICAS_NATIVAS_CIERRE_2026-03-22.md`
7. `EXPENSES_OLA6_SUBCATEGORIAS_REALES_CIERRE_2026-03-22.md`
8. `EXPENSES_CHECKLIST_MAESTRO_INTEGRACION_2026-03-22.md`
9. `EXPENSES_PROPUESTA_REFACTORIZACION_TAXONOMIA_Y_RUNTIME_2026-03-22.md`
10. `EXPENSES_WORKSPACE_OLA_CIERRE_2026-03-21.md`
11. `EXPENSES_OLA_REQUESTS_CIERRE_2026-03-21.md`
12. `EXPENSES_OLA_SETTINGS_CIERRE_2026-03-21.md`
13. `EXPENSES_OLA_SETTINGS_HARDENING_CIERRE_2026-03-21.md`
14. `EXPENSES_OLA5_FRONTEND_CIERRE_2026-03-21.md`
15. `EXPENSES_OLA7_MIRROR_AND_RELEASE_CHECK_2026-03-21.md`

## Regla de espejo

Todo cierre operativo de Expenses en backend debe replicarse en frontend con:

- mismo nombre de archivo
- misma fecha de corte
- misma version de contenido
- hash equivalente cuando aplique
