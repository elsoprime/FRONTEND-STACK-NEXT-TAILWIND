...
function InventoryAlertsContent({ tenantId, setLastTraceId }: InventoryAlertsContentProps) {
  const alertsQuery = useQuery({
    ...
    queryFn: async () => listInventoryLowStockAlerts(tenantId, { page: 1, limit: 10 }),
  });
    ...
    <div className="space-y-4">
      ...
      {alerts.length === 0 ? (
        ...
        <div className="space-y-2">
          {alerts.map((alert) => (
            <div key={alert.item.id} className="rounded-lg border border-border/80 bg-background/70 p-3">
